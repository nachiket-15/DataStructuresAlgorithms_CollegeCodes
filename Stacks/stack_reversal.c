...
void insertAtBottom(struct Node** top_ref, int item)
{
	if (isEmpty(*top_ref))
		...
		// Hold all items in Function Call Stack until we reach end of the stack. When the stack becomes empty, the isEmpty(*top_ref)become true, the above if part is executed and the item is inserted at the bottom
...
void reverse(struct Node** top_ref)
{
	if (!isEmpty(*top_ref)) {
		...
		// Function Call Stack)
		// one by one from the bottom to top. Every item inserted at the bottom
