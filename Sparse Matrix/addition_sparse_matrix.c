...
void create_new_node(struct Node **start, int row_no, int col_no, int value)
{
    ...
    {
        // there can be multiple nodes already present , but we want to add at last , so we will first traverse till end and then perform required opeartion of node creation and value updation
...
int main()
{
    ...
    // Assume 4x5 sparse matrix 1 - CREATE & PRINT
    printf("\nSparse matrix 1 is: \n\n");
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            ...
            if (Matrix_1[i][j] != 0)
                create_new_node(&head1, i, j, Matrix_1[i][j]);
        }
    }
    ...
    // Assume 4x5 sparse matrix 2 - CREATE & PRINT
    printf("\nSparse matrix 2 is: \n\n");
    int Matrix_2[4][5] =
        {
            {0, 0, 3, 0, 4},
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            // Pass only those values which are non-zero
            if (Matrix_2[i][j] != 0)
                create_new_node(&head2, i, j, Matrix_2[i][j]);
        }
    }
